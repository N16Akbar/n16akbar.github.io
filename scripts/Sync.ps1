...
# ---------------------------------------------------------
# 1. LOCAL EXTERNAL HDD BACKUPS (D:\00_Backup)
# ---------------------------------------------------------
...
if ($null -ne $drive) {
    ...
    rclone sync "$env:USERPROFILE\Documents\Encrypted Sync\Personal Vault" "D:\00_Backup\Personal Vault" -P

    Write-Host "Menyalin AutoHotkey ke External HDD..."
    rclone copy "$env:USERPROFILE\Documents\AutoHotkey" "D:\00_Backup\AutoHotkey" -P

    Write-Host "Sinkronisasi Wallpaper ke External HDD..."
    rclone sync "$env:USERPROFILE\Pictures\Wallpaper" "D:\00_Backup\Wallpaper" -P

    Write-Host "Sinkronisasi Screenshots ke External HDD..."
    rclone sync "$env:USERPROFILE\Pictures\Screenshots" "D:\00_Backup\Screenshots" -P --exclude "desktop.ini" --delete-excluded

    Write-Host "Sinkronisasi Music ke External HDD..."
    rclone sync "$env:USERPROFILE\Music" "D:\00_Backup\Music" -P --exclude "desktop.ini" --delete-excluded

    Write-Host "Sinkronisasi Keyfile ke External HDD..."
    rclone sync "$env:USERPROFILE\Documents\Keyfile" "D:\00_Backup\Keyfile" -P --exclude ".stfolder/" --delete-excluded

    Write-Host "Sinkronisasi Private ke External HDD..."
    rclone sync "$env:USERPROFILE\Documents\Private" "D:\00_Backup\Private" -P --exclude ".stfolder/" --delete-excluded

    Write-Host "Sinkronisasi PCSX2 ke External HDD..."
    rclone sync "$env:USERPROFILE\Documents\PCSX2" "D:\00_Backup\PCSX2" --include '/{bios/**,gamesettings/**,memcards/**,inputprofiles/**,snaps/**,inis/PCSX2.ini,inis/playtime.dat,inis/secrets.ini}' -P

    Write-Host "Sinkronisasi Obsidian Vault ke External HDD..."
    rclone sync "$env:USERPROFILE\Documents\Obsidian Vault" "D:\00_Backup\Obsidian Vault" -P --exclude ".stfolder/" --exclude ".stignore" --exclude ".obsidian/" --delete-excluded

    Write-Host "Sinkronisasi Czkawka ke External HDD..."
    rclone sync "$env:APPDATA\Qarmin\Czkawka\config" "D:\00_Backup\Config App Backup\PC\Czkawka" -P

    Write-Host "Sinkronisasi Ludusavi ke External HDD..."
    rclone sync "$env:APPDATA\ludusavi" "D:\00_Backup\Config App Backup\PC\Ludusavi" --include "config.yaml*" -P

    Write-Host "Sinkronisasi Save Data Backup ke External HDD..."
    rclone sync "$env:USERPROFILE\Documents\Save Data Backup" "D:\00_Backup\Save Data Backup" -P
} else {
    Write-Host "Drive HDD is not detected."
}
...
& $rcloneextra sync "$env:USERPROFILE\Documents\PCSX2" terabox:"Emulation\PCSX2" --include '/{bios/**,gamesettings/**,memcards/**,inputprofiles/**,snaps/**,inis/PCSX2.ini,inis/playtime.dat,inis/secrets.ini}' -P --tpslimit 1.5
